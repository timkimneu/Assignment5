...
public class SatEventFrame extends JFrame implements ScheduleSystemView<SatDOTW>,
  ...
  private IEvent<SatDOTW> originalEvent;
  private IEvent<SatDOTW> unmodifiedEvent;
   ...
   */
  public SatEventFrame(IReadOnlyPlannerModel<SatDOTW> model) {
    super();
    this.model = model;
  ...
  private SatEventImpl createEvent() {
    return new SatEventImpl(eventText.getText(),
            new SatTimeImpl(SatDOTW.valueOf(Objects.requireNonNull(startDOTW.getSelectedItem())
                    .toString().toUpperCase()), startTimeTxt.getText(),
                    (SatDOTW.valueOf(Objects.requireNonNull(endDOTW.getSelectedItem()).toString()
                            .toUpperCase())), endTimeTxt.getText()),
